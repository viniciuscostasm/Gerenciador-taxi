...
$header = new GHeader('Importação de Funcionários');
...
$html .= $form->addInput('hidden', 'p__emi_dat_date', false, array('value' => ''), false, false, false);
$html .= '<div id="p__emi_dat_date_text" class="btn sepV_b"><i class="icon-calendar"></i>&nbsp;<span>' . $emi_dat_dateText . '</span>&nbsp;&nbsp;<b class="caret"></b></div>';
...
$paginate = new GPaginate('employee', 'employee_load.php', SYS_PAGINACAO);
$html .= $paginate->get();
$html .= '</div>'; //divTable
...
?>
<script>
    var pagCrud = 'employee_crud.php';
    var pagView = 'employee_view.php';
    var pagLoad = 'employee_load.php';
    var pagForm = 'employee_form.php';

    function filtrar(page) {
        employeeLoad('', '', '', $('#filter').serializeObject(), page);
        return false;
    }
    ...
    $(function() {
        ...
        $('#p__btn_limpar').click(function() {
            clearForm('#filter');
            $('#p__emi_dat_date_text span').html((moment().subtract(89, 'days')).format('DD/MM/YYYY') + ' até ' + (moment()).format('DD/MM/YYYY'));
            $('#p__emi_dat_date').val((moment().subtract(89, 'days')).format('DD/MM/YYYY') + '-' + (moment()).format('DD/MM/YYYY'));
            filtrar(1);
        });
        ...
        $('#p__emi_dat_date_text').daterangepicker({
            ranges: {
                'Hoje': [moment(), moment()],
        ...
        function(start, end) {
            if (start.toString('dd/MM/yyyy') !== end.toString('dd/MM/yyyy')) {
                $('#p__emi_dat_date_text span').html(start.format('DD/MM/YYYY') + ' até ' + end.format('DD/MM/YYYY'));
                $('#p__emi_dat_date').val(start.format('DD/MM/YYYY') + '-' + end.format('DD/MM/YYYY'));
            } else {
                $('#p__emi_dat_date_text span').html(start.format('DD/MM/YYYY'));
                $('#p__emi_dat_date').val(start.format('DD/MM/YYYY'));
            }
            filtrar(1);
        });
    });

    function loadEmployee(emi_int_id) {
        $.gAjax.load(pagForm, {emi_int_id:emi_int_id}, '#loadEmployee', function (response) {
