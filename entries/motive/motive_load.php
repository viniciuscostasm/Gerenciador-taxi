...
$mot_var_name = $_POST['p__mot_var_name'];

if (!empty($mot_var_name)) {
    $filter->addFilter('AND', 'mot_var_name', 'LIKE', 's', '%' . str_replace(' ', '%', $mot_var_name) . '%');
}
//-------------------------------- Filtros -----------------------------------//

try {
    if ($type == 'C') {
        $query = "SELECT count(1) FROM vw_tax_motive " . $filter->getWhere();
        ...
        $filter->setOrder(array('mot_var_name' => 'ASC'));
        $filter->setLimit($start, $rp);

        $query = "SELECT mot_int_id, mot_var_name FROM vw_tax_motive " . $filter->getWhere();
        $param = $filter->getParam();
        $mysql->execute($query, $param);
        ...
        if ($mysql->numRows() > 0) {
            ...
            $html .= '<tbody>';
            while ($mysql->fetch()) {
                $class = ($_POST['p__selecionado'] == $mysql->res['mot_int_id']) ? 'success' : '';
                $html .= '<tr id="' . $mysql->res['mot_int_id'] . '" class="linhaRegistro ' . $class . '">';
                $html .= '<td>' . $mysql->res['mot_var_name'] . '</td>';
